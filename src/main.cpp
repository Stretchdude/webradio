...
char title[128];         // character array to hold meta data message
bool newTitle = false;
...
void setup()
{
  ...
  if (pref.isKey("ntp"))
    ntp = pref.getString("ntp");
  curGain = 40; // default value
  if (pref.isKey("gain"))
    curGain = pref.getUShort("gain");
  curStation = 0; // default value
  // set current station to saved value if available
  if (pref.isKey("station"))
    curStation = pref.getUShort("station");
  if (curStation >= STATIONS)
    curStation = 0;        // check to avoid invalid station number
  ...
  connected = initWiFi(STASSID, STAPSK);
  if (connected)
...
void loop()
{
  ...
  if (!connected && ((millis() - discon) > 300000))
    ESP.restart();
}
// Das geht nicht.nimm arduino IDE
